...
def panorama():
	...
	indice = st.selectbox('Selecione o índice', dict_tickers.keys())
	...
	fig2 = go.Figure(data=[go.Candlestick(x=hist_acao.index,
				open=hist_acao['Open'],
				high=hist_acao['High'],
				low=hist_acao['Low'],
				close=hist_acao['Close'])])
	fig2.update_layout(title=acao, xaxis_rangeslider_visible=False)
...
def mapa_mensal():
	...
	if analisar:
		...
		if opcao == 'Indices':
			retornos = inv.get_index_historical_data(ticker, country='brazil', from_date=data_inicial, to_date=data_final, interval='Monthly')['Close'].pct_change()
		else:
			retornos = inv.get_stock_historical_data(ticker, country='brazil', from_date=data_inicial, to_date=data_final, interval='Monthly')['Close'].pct_change()	
		...
		#Criar tabela matriz de retornos
...
def fundamentos():
	import fundamentus as fd  #https://github.com/mv/fundamentus-api
...
def main():
	st.sidebar.image('grafico_logo.jpg', width=200)
